...
OPTION_ATTRS = %i{dt ticker call month_remaining volatility commodity name file_name volume underlying_price line_open_interest future_open_interest tnote_rate}
...
File.open('options.csv', 'w') do |output|
  ...
  Dir.glob('market/**/*.txt')
    ...
    .flat_map(&:options)
    .sort_by { |o| "#{o.dt}-#{o.commodity}-#{o.call}-#{o.ticker}-#{o.month_remaining}" }
    .each do |option|
